<template>
    ...
    <a-drawer v-model:visible="visible" class="custom-class" style="color: red" title="Tipo propietario" placement="right"
...
import { addNewType, updateType } from '../composables/TypeOwner';
...
const submitObject = async () => {
    ...
    try {
        let result = await addNewType(item);
        successNotification("Registro exitoso", `El tipo de propietario ${item.name.toUpperCase()} ha sido registrado.`);
        emit('update-list');
    } catch (e) {
        if (e.response) {
            ...
            errorNotification("Ocurrió un error el Tipo de propietario :(", data.message);
        }
    } finally {
        hideDrawer();
        cleanObject();
...
const editObject = async () => {
    const item = { ...object }; // this is to prevent reactivity
    try {
        const result = await updateType(item, objectToUpdate.value.id);
